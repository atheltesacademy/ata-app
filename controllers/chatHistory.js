...
exports.createChat = async (req, res) => {
    try {
        ...
        const chat = await ChatHistory.create({ chat_id, participant_id, message }); 
...
exports.getCoachChats = async (req, res) => {
    try {
        ...
        const chatHistory = await ChatHistory.find({ participant_id: coach_id }); // Use ChatHistory instead of chatHistory
...
exports.getAthleteChats = async (req, res) => {
    try {
        ...
        const chatHistory = await ChatHistory.find({ participant_id: athlete_id }); // Use ChatHistory instead of chatHistory
        res.status(200).json({ success: true, chatHistory });
    } catch (error) {
        res.status(500).json({ success: false, message: error.message });
    }
