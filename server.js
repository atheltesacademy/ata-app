...
io.on('connection', (socket) => {
  ...
  socket.on('message', async (data) => {
    ...
    try {
      ...
      } else if (!chat.chats || chat.chats.length === 0) {
        // If 'chats' array is missing or empty, initialize it with an empty object
        chat.chats = [{}];
      }

      // Ensure that 'messages' array exists in the chat object
      if (!chat.chats[0].messages) {
